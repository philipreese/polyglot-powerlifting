...
- [ ] **Git over SSH:** Generate an SSH keypair (`ssh-keygen`), add the public key to GitHub, and clone the empty repo using the `git@github.com` URL instead of HTTPS. Understand the difference.
...
- [ ] **Environment:** Create `.devcontainer/` so your tools (Node, Python, Flutter) are consistent
- [ ] **Database:** Set up Supabase free tier, create `lifts` table schema, enable Auth
- [ ] **DX:** Add Prettier/ESLint configs for JS/TS, Ruff for Python

### Phase 1 — FastAPI Backend (The Foundation)
- [ ] **Branching:** Create branch `feat/fastapi-core` from `main`
- [ ] **Core:** Implement endpoints based on your OpenAPI spec
...
- [ ] **Merge:** Open a Pull Request, review your own diff, merge `feat/fastapi-core` into `main`
- [ ] **CI/CD:** Set up GitHub Actions on `main` to run Ruff (linting) and pytest, then deploy to Render
